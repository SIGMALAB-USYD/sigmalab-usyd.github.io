...
function conferencePubItemConstructor(pubitem) {
  ...
  for (main_image of pubitem.main_images) {
    image_list += `<img alt="" border="0" src="${main_image}" width="250px" height="200" >`;
  }
...
for (pubitem of conference_data.reverse()) {
  papers = []
  for (pub of pubitem.papers){
    if('source_code' in pub)
    {
      papers.push(pub);
    }
    pubitem.papers = papers
  }
